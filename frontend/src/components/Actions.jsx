...
const Actions = ({post }) => {
	...
	const handleReply = async()=>{
		if(!user){
			showToast("", "Log in to reply", "error")
			return
		}
		if(isRepling)return
		setIsRepling(true)
		try {
			...
			setPosts(updatedPost)
			showToast("", "Replied Added!", "success")
			setReply("")
			onClose()
		} catch (error) {
			showToast("", "error", "error")
        ...
        <ModalContent bg={useColorModeValue('white', 'gray.dark')}>
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input placeholder='Reply to ' value={reply} onChange={(e)=>(setReply(e.target.value))} />
            </FormControl>
          </ModalBody>

          <ModalFooter>
            <Button colorScheme='blue' mr={3} size={"sm"} onClick={handleReply} isLoading={isRepling}>
              Reply
            </Button>
          </ModalFooter>
        </ModalContent>
